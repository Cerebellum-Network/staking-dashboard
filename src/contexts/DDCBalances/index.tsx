...
export const DDCBalancesContext =
  React.createContext<DDCBalancesContextInterface>(
    defaults.defaultBalancesContext
  );

export const useDDCBalances = () => React.useContext(DDCBalancesContext);

export const DDCBalancesProvider = ({
  children,
}: {
  ...
  const subscribeToLedger = async (address: string) => {
    ...
    const unsub: () => void = await api.queryMulti<[AnyApi]>(
      [[api.query.ddcStaking.ledger, address]],
